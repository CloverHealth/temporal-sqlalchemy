...
from temporal_sqlalchemy.clock import build_history_table, build_history_class, build_clock_class
...
def test_build_history_table():
    ...
    assert history_table == build_history_table(rel_prop, models.TEMPORAL_SCHEMA)
    assert history_table.name == 'relational_temporal_history_rel_id'
    assert history_table.schema == models.TEMPORAL_SCHEMA
    assert history_table.c.keys() == ['id', 'effective', 'vclock', 'entity_id', 'rel_id']
...
def test_build_clock_class():
    clock = build_clock_class('Testing', sa.MetaData(), {'__tablename__': 'test'})
