...
class TestTemporalModels(shared.DatabaseTest):
    ...
    def test_temporal_options_class(self):
        ...
        assert clock_table.__table__.name == '%s_clock' % models.SimpleTableTemporal.__table__.name
    ...
    def test_temporal_options_instance(self):
        ...
        assert temp.temporal_options is models.SimpleTableTemporal.temporal_options
    ...
    def test_creates_temporal_tables(self, session):
        ...
        assert self.has_table(session.bind, '%s_clock' % table_name, schema=models.TEMPORAL_SCHEMA)
        assert self.has_table(session.bind, '%s_history_prop_a' % table_name, schema=models.TEMPORAL_SCHEMA)
        assert self.has_table(session.bind, '%s_history_prop_b' % table_name, schema=models.TEMPORAL_SCHEMA)
    
    def test_init_adds_clock_tick(self, session):
        clock_query = session.query(models.SimpleTableTemporal.temporal_options.clock_table).count()
        ...
        clock_query = session.query(models.SimpleTableTemporal.temporal_options.clock_table)
        ...
        # more explicit than looping over SimpleTableTemporal.temporal_options.history_tables.values()
        prop_a_history_model = temporal.get_history_model(models.SimpleTableTemporal.prop_a)
        prop_b_history_model = temporal.get_history_model(models.SimpleTableTemporal.prop_b)

        for attr, history_table in zip(('prop_a', 'prop_b'), (prop_a_history_model, prop_b_history_model)):
    ...
    def test_clock_tick_editing(self, session):
        ...
        clock = clock_query\
            .order_by(models.SimpleTableTemporal.temporal_options.clock_table.tick.desc())\
            .first()
        for history_table in models.SimpleTableTemporal.temporal_options.history_tables.values():
            clock_query = session.query(history_table).count()
    ...
    def test_clock_table_has_no_activity_columns(self):
        ...
        assert 'activity_id' not in models.SimpleTableTemporal.temporal_options.clock_table.__dict__
        assert 'activity' not in models.SimpleTableTemporal.temporal_options.clock_table.__dict__
    ...
    def test_multiple_edits(self, session):
        history_tables = {
            'prop_a': temporal.get_history_model(models.SimpleTableTemporal.prop_a),
            'prop_b': temporal.get_history_model(models.SimpleTableTemporal.prop_b),
        }
    ...
    def test_doesnt_duplicate_unnecessary_history(self, session):
        history_tables = {
            'prop_a': temporal.get_history_model(models.SimpleTableTemporal.prop_a),
            'prop_b': temporal.get_history_model(models.SimpleTableTemporal.prop_b),
            'prop_c': temporal.get_history_model(models.SimpleTableTemporal.prop_c),
        }
