...
class TestTemporalWithActivity(shared.DatabaseTest):
    ...
    def test_clock_table_has_activity_id(self):
        assert isinstance(
            models.FirstTemporalWithActivity.temporal_options.clock_table.activity_id,
            orm_attr.InstrumentedAttribute)
        assert isinstance(
            models.FirstTemporalWithActivity.temporal_options.clock_table.activity,
            orm_attr.InstrumentedAttribute)
    ...
    def test_activity_on_entity_create(self, session):
        ...
        activity_clock_history = temporal.get_activity_clock_backref(activity_result, models.FirstTemporalWithActivity)
        ...
        clock_query = session.query(models.FirstTemporalWithActivity.temporal_options.clock_table)
    ...
    def test_no_activity_on_entity_edit(self, session):
        ...
        t = models.FirstTemporalWithActivity(column=1234, activity=create_activity)
        session.add(t)
        session.commit()
    ...
    def test_activity_on_entity_edit(self, session):
        ...
        activity_clock1_backref = temporal.get_activity_clock_backref(models.Activity, models.FirstTemporalWithActivity)
        assert getattr(create_activity_result, activity_clock1_backref.key, False)
        ...
        assert getattr(edit_activity_result, activity_clock1_backref.key, False)
